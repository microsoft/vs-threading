	...
	public class AsyncReaderWriterLockTests {
		...
		public async Task UpgradeReadLocksMutuallyExclusive() {
			...
			await Task.WhenAll(
				Task.Run(async delegate {
				using (await this.asyncLock.UpgradeableReadLockAsync()) {
					firstUpgradeableReadHeld.SetAsync();
				...
				secondUpgradeableReadBlocked.SetAsync();
			}),
		...
		public async Task UpgradeableReadAvailableWithExistingReaders() {
			...
			await Task.WhenAll(
				...
				using (await this.asyncLock.UpgradeableReadLockAsync()) {
					upgradeableReaderHasLock.SetAsync();
				}
			})
				);
		}
			...
			await Task.WhenAll(
				...
				using (await this.asyncLock.UpgradeableReadLockAsync()) {
					...
					using (await this.asyncLock.WriteLockAsync()) {
						...
						upgradeableReadUpgraded.SetAsync();
					}
				}
			}),
				Task.Run(async delegate {
		...
		public async Task UpgradeableReaderWaitsForExistingReadersToExit() {
			...
			await Task.WhenAll(
				...
				using (await this.asyncLock.UpgradeableReadLockAsync()) {
					...
					upgradeableReaderWaitingForUpgrade.SetAsync();
				}
			}),
				Task.Run(async delegate {
				using (await this.asyncLock.ReadLockAsync()) {
		...
		public async Task NestedReadersStillIssuedLocksWhileWaitingWriters() {
			...
			await Task.WhenAll(
				Task.Run(async delegate {
				using (await this.asyncLock.ReadLockAsync()) {
					readerLockHeld.SetAsync();
					await writerQueued.Task;

					using (await this.asyncLock.ReadLockAsync()) {
						Assert.IsFalse(writerLockHeld.Task.IsCompleted);
						readerNestedLockHeld.SetAsync();
					}
				}
			}),
				Task.Run(async delegate {
